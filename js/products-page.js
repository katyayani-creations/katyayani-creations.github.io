...
class ProductsPage {
  ...
  async loadProducts() {
    // API removed - products are now loaded server-side via PHP
    // This method is no longer needed as products are rendered in HTML

    // Get products from existing DOM elements if needed for filtering
    this.loadProductsFromDOM();
  }

  loadProductsFromDOM() {
    // Extract product data from rendered HTML for client-side filtering
  ...
  applySorting() {
    switch (this.filters.sort) {
      ...
      case "featured":
        this.filteredProducts.sort((a, b) => {
          // Assuming featured products have a featured property or are from featured list
...
// Global function for clear filters button in no-products section
...
// Debug function to test sort functionality
function debugSort() {
  const container = document.getElementById("products-grid");
...
// Simpler DOM-based filtering for existing products
...
// Count only visible products (not hidden by search)
...
// Initialize simple filtering when DOM is loaded
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Initialize ProductsPage class for other functionality
